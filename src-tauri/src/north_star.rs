...
use crate::{
    ...
    error::AppError,
    models::{
        AppSettings, CallTurnResult, LocationEventInput, NorthStarCallReview, NorthStarCallSession,
        NorthStarCallTurn, NorthStarDesktopBinding, NorthStarLocationEvent, NorthStarMessage,
...
fn ensure_session_config(settings: &AppSettings) -> Result<(), AppError> {
    if settings.north_star_user_handle.trim().is_empty() {
        return Err(AppError::Message(
            "North Star user handle is missing.".into(),
        ));
    }
    if settings.north_star_display_name.trim().is_empty() {
        return Err(AppError::Message(
            "North Star display name is missing.".into(),
        ));
    }
    Ok(())
}
...
fn auth_request(
    ...
    if settings.north_star_session_token.trim().is_empty() {
        return Err(AppError::Message(
            "North Star session is not ready yet.".into(),
        ));
    }
    let url = format!("{}{}", endpoint(settings)?, path);
    Ok(client.request(method, url).header(
        "x-northstar-session",
        settings.north_star_session_token.as_str(),
    ))
}
...
pub fn bind_desktop(settings: &mut AppSettings) -> Result<NorthStarSnapshot, AppError> {
    if settings.north_star_session_token.trim().is_empty() {
        return Err(AppError::Message(
            "Create a North Star session first.".into(),
        ));
    }
    ...
    fetch_snapshot(
        settings,
        Some(format!(
            "Desktop bound to North Star as {}.",
            response.status
        )),
    )
}
...
pub fn send_heartbeat(settings: &AppSettings) -> Result<NorthStarSnapshot, AppError> {
    if settings.north_star_device_token.trim().is_empty() {
        return Err(AppError::Message(
            "Bind the desktop to North Star first.".into(),
        ));
    }

    client()
        .post(format!(
            "{}/api/companion/desktop-heartbeat",
            endpoint(settings)?
        ))
    ...
    fetch_snapshot(
        settings,
        Some("Sent desktop heartbeat to North Star.".into()),
    )
}
...
pub fn send_desktop_message(
    ...
    if trimmed.is_empty() {
        return Err(AppError::Message(
            "North Star message text is empty.".into(),
        ));
    }

    client()
        .post(format!(
            "{}/api/companion/messages/from-desktop",
            endpoint(settings)?
        ))
...
pub fn send_call_request(
    ...
    client()
        .post(format!(
            "{}/api/companion/call-sessions/from-desktop",
            endpoint(settings)?
        ))
...
pub fn request_location_pulse(settings: &AppSettings) -> Result<NorthStarSnapshot, AppError> {
    if settings.north_star_device_token.trim().is_empty() {
        return Err(AppError::Message(
            "Bind the desktop to North Star first.".into(),
        ));
    ...
    client()
        .post(format!(
            "{}/api/companion/location-pulses/from-desktop",
            endpoint(settings)?
        ))
    ...
    fetch_snapshot(
        settings,
        Some("Asked North Star for a fresh location pulse.".into()),
    )
}
...
pub fn fetch_snapshot(
    ...
    let configured =
        !endpoint_value.is_empty() && !settings.north_star_user_handle.trim().is_empty();
    ...
    if settings.north_star_session_token.trim().is_empty() {
        return Ok(NorthStarSnapshot {
            ...
            detail: detail_override
                .unwrap_or_else(|| "North Star session has not been created yet.".into()),
        });
    }

    let state = auth_request(
        &client(),
        settings,
        reqwest::Method::GET,
        "/api/companion/state",
    ...
    let locations = auth_request(
        &client(),
        settings,
        reqwest::Method::GET,
        "/api/companion/location-events",
    )?
    .send()?
    .error_for_status()?
    .json::<LocationEventsResponse>()?;

    let calls = auth_request(
        &client(),
        settings,
        reqwest::Method::GET,
        "/api/companion/call-sessions",
    )?
    .send()?
    .error_for_status()?
    .json::<CallSessionsResponse>()?;

    let reviews = auth_request(
        &client(),
        settings,
        reqwest::Method::GET,
        "/api/companion/call-reviews",
    )?
    .send()?
    .error_for_status()?
...
pub fn fetch_runtime_snapshot(
    ...
    let configured = !settings.north_star_endpoint.trim().is_empty()
        && !settings.north_star_user_handle.trim().is_empty();
...
pub fn latest_accepted_call(
    settings: &AppSettings,
) -> Result<Option<NorthStarCallSession>, AppError> {
    let snapshot = fetch_snapshot(settings, None)?;
    let preferred_device_token = settings.north_star_device_token.trim();
    Ok(snapshot.call_sessions.into_iter().find(|call| {
...
pub fn end_latest_accepted_call(settings: &AppSettings) -> Result<(), AppError> {
    ...
    auth_request(
        ...
        settings,
        reqwest::Method::POST,
        "/api/companion/call-sessions/respond",
    )?
...
pub fn send_desktop_webrtc_signal(
    ...
    if trimmed_signal_kind.is_empty() {
        return Err(AppError::Message(
            "North Star signal kind is missing.".into(),
        ));
    }
    let trimmed_payload = payload_json.trim();
    if trimmed_payload.is_empty() {
        return Err(AppError::Message(
            "North Star signal payload is empty.".into(),
        ));
    }
...
pub fn fetch_rtc_config(settings: &AppSettings) -> Result<Vec<NorthStarRtcIceServer>, AppError> {
    ...
    let response = client()
        ...
        .header(
            "x-northstar-session",
            settings.north_star_session_token.clone(),
        )
        .send()?
        .error_for_status()?
