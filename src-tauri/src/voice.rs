...
use crate::{
    error::AppError,
    models::{
        AppSettings, CallTurnResult, NorthStarLiveReplyStreamEvent, VoiceSnapshot,
...
const KOKORO_FASTAPI_VOICE_BASE_URL: &str =
    "https://huggingface.co/hexgrad/Kokoro-82M/resolve/main/voices";
const KOKORO_MIN_VOICE_FILE_BYTES: u64 = 1024;
const KOKORO_AVAILABLE_VOICES: &[&str] = &[
    "af_alloy",
    "af_aoede",
    "af_bella",
    "af_heart",
...
const MOONSHINE_MODEL_BASE_URL: &str =
...
const DEFAULT_CALL_REPLY: &str =
    "I had a little trouble with that reply just then. Can you ask it once more?";
...
const DEFAULT_CALL_REPLY_MODEL_ISSUE: &str =
    "I had a little trouble responding just then. Can you say that again?";
...
pub fn voice_snapshot(
    ...
    let runtime_ready = if tts_uses_remote_kokoro {
        match managed_fastapi_health(settings) {
            ...
            _ => kokoro_runtime_slot
                .lock()
                .map_err(|_| {
                    AppError::Message("Kokoro-FastAPI runtime mutex was poisoned.".into())
                })?
                .as_mut()
    ...
    let example_voices = if tts_uses_remote_kokoro {
        let discovered = list_managed_fastapi_voices(&managed_root);
    ...
    Ok(VoiceSnapshot {
        ...
        runtime_detail: if tts_uses_remote_kokoro {
            if settings.tts_endpoint.trim().is_empty() {
                ...
                format!(
                    "Managed Kokoro-FastAPI is reachable at {}.",
                    settings.tts_endpoint.trim()
                )
            } else {
                format!(
                    "Kokoro-FastAPI is configured for {} but is not reachable yet.",
                    settings.tts_endpoint.trim()
                )
            }
        ...
        runtime_stdout_log: managed_logs_dir
            .join("kokoro-fastapi.stdout.log")
            .display()
            .to_string(),
        runtime_stderr_log: managed_logs_dir
            .join("kokoro-fastapi.stderr.log")
            .display()
            .to_string(),
        speech_ready,
...
pub fn prepare_python_runtime(
    ...
    let python =
        python_command().ok_or_else(|| AppError::Message("Could not find Python 3.".into()))?;
...
pub fn setup_local_speech(
    ...
    download_moonshine_model_if_missing(&paths.speech_model_dir).and_then(|_| {
...
fn ensure_speech_worker<'a>(
    ...
    *speech_worker_slot = None;
    ...
    if read_count == 0 {
        return Err(AppError::Message(
            "Speech worker did not start correctly.".into(),
        ));
    }
    ...
    *speech_worker_slot = Some(SpeechStreamWorker {
        child,
        stdin,
        snapshot,
    });
...
fn send_speech_worker_command(
    worker: &mut SpeechStreamWorker,
    payload: &Value,
) -> Result<(), AppError> {
    ...
    F: Fn(&SpeechStreamSnapshotState) -> bool,
{
    ...
    loop {
        ...
        if started.elapsed() >= timeout {
            return Err(AppError::Message(
                "Speech stream state transition timed out.".into(),
            ));
        }
...
pub fn start_speech_stream(
    ...
    if existing_snapshot.active
        || matches!(
            existing_snapshot.status.as_str(),
            "starting" | "listening" | "processing"
        )
    {
...
pub fn stop_speech_stream_and_reply(
    ...
    if transcript_text.is_empty() {
        return Err(AppError::Message(
            "Speech stream produced no transcript.".into(),
        ));
    }
...
pub fn run_call_turn(
    ...
    let script_path = ensure_python_script(app_data_dir)?;
    fs::create_dir_all(&paths.recordings_dir)?;
    let recording_path = paths.recordings_dir.join(format!(
        "call-input-{}-{}.wav",
        session_id,
...
pub fn synthesize_test_phrase(
    ...
    let voice_name = if settings.tts_default_voice.trim().is_empty() {
        "af_heart"
    } else {
        settings.tts_default_voice.trim()
    };
    let audio_bytes = synthesize_text_once(
        app_data_dir,
        settings,
        worker_slot,
        kokoro_runtime_slot,
...
fn build_call_turn_result(
    ...
    let spoken_transcript = transcript_text.trim().to_string();
    let (reply_text, reply_mode) =
        choose_call_reply(settings, session_context, &spoken_transcript)?;
    let reply_voice = if settings.tts_default_voice.trim().is_empty() {
        "af_heart"
    } else {
        settings.tts_default_voice.trim()
    };
    ...
    Ok(CallTurnResult {
        session_id,
        transcript_text: spoken_transcript,
    ...
    F: FnMut(NorthStarLiveReplyStreamEvent) -> Result<(), AppError>,
{
    ...
    emit_event(NorthStarLiveReplyStreamEvent {
        ...
        transcript_text: Some(spoken_transcript.clone()),
        reply_text: None,
        reply_mode: None,
        text_chunk: None,
        audio_base64: None,
    ...
    let full_audio_bytes = synthesize_text_once(
        app_data_dir,
        settings,
        worker_slot,
        kokoro_runtime_slot,
    ...
    let result = CallTurnResult {
        session_id,
        transcript_text: spoken_transcript.clone(),
        reply_text: reply_text.clone(),
    ...
    emit_event(NorthStarLiveReplyStreamEvent {
        request_id: request_id.to_string(),
        phase: "complete".into(),
        transcript_text: Some(spoken_transcript),
...
fn synthesize_text_once(
    ...
    match synthesize_text_with_worker(
        app_data_dir,
        settings,
        worker_slot,
        &cleaned_text,
...
fn tts_uses_fastapi(settings: &AppSettings) -> bool {
    ...
    provider == "kokoro_fastapi"
        ...
        || (provider == "kokoro" && !settings.tts_endpoint.trim().is_empty())
}
...
fn managed_fastapi_health(settings: &AppSettings) -> Result<bool, AppError> {
    ...
    let root = normalize_tts_endpoint(endpoint)
        .trim_end_matches("/v1")
        .to_string();
...
fn managed_fastapi_voice_dir(root: &Path) -> PathBuf {
    root.join("api").join("src").join("voices").join("v1_0")
}

fn list_managed_fastapi_voices(root: &Path) -> Vec<String> {
    let voice_dir = managed_fastapi_voice_dir(root);
    let mut voices = Vec::new();
    if let Ok(entries) = fs::read_dir(&voice_dir) {
        for entry in entries.flatten() {
            let path = entry.path();
            let is_valid_pt = path
                .extension()
            ...
            if metadata.len() < KOKORO_MIN_VOICE_FILE_BYTES {
                continue;
            }
...
fn ensure_managed_kokoro_fastapi_installed(app_data_dir: &Path) -> Result<PathBuf, AppError> {
    ...
    let parent = root
        .parent()
        .ok_or_else(|| AppError::Message("Managed Kokoro-FastAPI path was invalid.".into()))?;
    fs::create_dir_all(parent)?;
    let output = hidden_command("git")
        .args([
            ...
            "--depth",
            "1",
            KOKORO_FASTAPI_REPO_URL,
            root.to_string_lossy().as_ref(),
        ])
        .output()
        .map_err(|caught| {
            AppError::Message(format!(
                "Could not start git clone for Kokoro-FastAPI: {caught}"
            ))
        })?;
    if !output.status.success() {
...
fn ensure_managed_fastapi_voice_assets(root: &Path) -> Result<(), AppError> {
    let voice_dir = managed_fastapi_voice_dir(root);
    fs::create_dir_all(&voice_dir)?;
    let client = Client::builder().timeout(Duration::from_secs(90)).build()?;
    for voice in KOKORO_AVAILABLE_VOICES {
        let destination = voice_dir.join(format!("{voice}.pt"));
        let needs_download = match fs::metadata(&destination) {
            Ok(metadata) => metadata.len() < KOKORO_MIN_VOICE_FILE_BYTES,
            Err(_) => true,
        };
        if !needs_download {
            continue;
        }
        let url = format!("{KOKORO_FASTAPI_VOICE_BASE_URL}/{voice}.pt?download=true");
        download_file_force(&client, &url, &destination)?;
    }
    Ok(())
}
...
fn ensure_managed_kokoro_fastapi_runtime(
    ...
    ensure_managed_fastapi_voice_assets(&root)?;
    let python = python_command().ok_or_else(|| {
        AppError::Message("Could not find Python 3 for managed Kokoro-FastAPI.".into())
    })?;
    ...
    fs::write(
        &bootstrap_script,
        managed_fastapi_bootstrap_script(&root, &python),
    )?;
    ...
    Err(AppError::Message(
        "Managed Kokoro-FastAPI did not become ready in time.".into(),
    ))
}
...
fn synthesize_text_via_fastapi(
    ...
    if endpoint.is_empty() {
        return Err(AppError::Message(
            "Kokoro-FastAPI endpoint is empty.".into(),
        ));
    }
    ...
    if !response.status().is_success() {
        ...
        return Err(AppError::Message(format!(
            "Kokoro-FastAPI synthesis failed with {status}: {detail}"
        )));
    }
...
fn synthesize_text_with_worker(
        ...
        match ensure_voice_worker(app_data_dir, settings, &mut guard) {
            Ok(worker) => {
                ...
                let synthesis_result = {
                    if writeln!(worker.stdin, "{request_line}").is_err()
                        || worker.stdin.flush().is_err()
                    {
                        should_reset = true;
                        Err(AppError::Message(
                            "Voice worker stopped before synthesis completed.".into(),
                        ))
                    } else {
                        ...
                        match worker.stdout.read_line(&mut response_line) {
                            Ok(0) => {
                                should_reset = true;
                                Err(AppError::Message(
                                    "Voice worker closed unexpectedly.".into(),
                                ))
                            }
                            Ok(_) => {
                                    ...
                                    .unwrap_or(false)
                                {
                                    let audio_base64 = payload
                                        ...
                                        .ok_or_else(|| {
                                            AppError::Message(
                                                "Voice worker returned no audio.".into(),
                                            )
                                        })?;
                                    STANDARD.decode(audio_base64).map_err(|error| {
                                        AppError::Message(format!(
                                            "Voice worker returned invalid audio: {error}"
                                        ))
                                    })
                                } else {
                                    should_reset = true;
    ...
    Err(last_error
        .unwrap_or_else(|| AppError::Message("Voice worker could not synthesize audio.".into())))
}
...
fn synthesize_text_one_shot(
    ...
    if !paths.model_path.exists() || !paths.voices_path.exists() {
        return Err(AppError::Message(
            "Kokoro model files are missing. Download them first in Settings > Voice.".into(),
        ));
    }
    let python =
        python_command().ok_or_else(|| AppError::Message("Could not find Python 3.".into()))?;
...
fn ensure_voice_worker<'a>(
    ...
    *worker_slot = None;
    ...
    if read_count == 0 {
        return Err(AppError::Message(
            "Voice worker did not start correctly.".into(),
        ));
    }
...
fn generate_call_reply(
    ...
    let endpoint = settings
        ...
        .trim_end_matches('/')
        .to_string();
    let lane_aware_context = build_lane_aware_call_context(session_context, transcript_text);
    let outbound_call = is_outbound_outreach_call(session_context);
    if outbound_call && is_outreach_origin_question(transcript_text) {
    ...
    request_call_reply(
        settings,
        &endpoint,
        "Reply with plain spoken answer text only.",
        &fallback_prompt,
        0.3,
        96,
    )
    ...
    F: FnMut(NorthStarLiveReplyStreamEvent) -> Result<(), AppError>,
{
        ...
        F: FnMut(NorthStarLiveReplyStreamEvent) -> Result<(), AppError>,
    {
        ...
        let total_parts = audio_base64
            .len()
            .div_ceil(NORTH_STAR_LIVE_REPLY_AUDIO_SLICE_SIZE);
        for part_index in 0..total_parts {
            let start = part_index * NORTH_STAR_LIVE_REPLY_AUDIO_SLICE_SIZE;
            let end =
                ((part_index + 1) * NORTH_STAR_LIVE_REPLY_AUDIO_SLICE_SIZE).min(audio_base64.len());
            emit_event(NorthStarLiveReplyStreamEvent {
                request_id: request_id.to_string(),
                ...
                reply_text: None,
                reply_mode: Some(reply_mode.into()),
                text_chunk: Some(if part_index == 0 {
    ...
    let user_prompt = if outbound_call {
        render_call_prompt_template(
    ...
    thread::scope(|scope| -> Result<(String, String, Vec<String>), AppError> {
            ...
            F: FnMut(NorthStarLiveReplyStreamEvent) -> Result<(), AppError>,
        {
            loop {
                ...
                if block_until_next && *next_emit_index < queued_chunk_count {
                    let result = tts_result_rx.recv().map_err(|_| {
                        AppError::Message(
                            "Live reply TTS worker stopped before all chunks were synthesized."
                                .into(),
                        )
                    })?;
                    let (completed_index, completed_text, completed_audio) =
                ...
                match tts_result_rx.try_recv() {
                    Ok(result) => {
                        let (completed_index, completed_text, completed_audio) =
                            result.map_err(AppError::Message)?;
                        pending_tts_results
                            .insert(completed_index, (completed_text, completed_audio));
                    }
            ...
            F: FnMut(NorthStarLiveReplyStreamEvent) -> Result<(), AppError>,
        {
            ...
            *queued_chunk_count += 1;
            tts_request_tx.send((queued_index, text)).map_err(|_| {
                AppError::Message("Could not queue live reply chunk for synthesis.".into())
            })
        }
        ...
        loop {
            ...
            while let Some(segment) =
                next_streamable_segment(&visible_accumulated, &mut flushed_visible_prefix, false)
            {
                let cleaned = sanitize_text(&segment);
                if cleaned.is_empty() {
                    continue;
                }
                queue_stream_segment(
                    cleaned,
                    &mut queued_chunk_count,
                    &tts_request_tx,
                    request_id,
                    emit_event,
                )?;
        ...
        let visible_accumulated = sanitize_text(&strip_think_text(&accumulated));
        while let Some(segment) =
            next_streamable_segment(&visible_accumulated, &mut flushed_visible_prefix, true)
        {
            let cleaned = sanitize_text(&segment);
            if cleaned.is_empty() {
                continue;
    ...
    let mut normalized = cleaned
        ...
        .filter_map(|line| {
            ...
            if lowered.starts_with("flash think:") || lowered.starts_with("flash-think:") {
                return Some(
                    trimmed[trimmed.find(':').map(|idx| idx + 1).unwrap_or(0)..]
                        .trim()
                        .to_string(),
                );
            }
...
fn is_outreach_origin_question(transcript_text: &str) -> bool {
    ...
    (lowered.contains("why are you calling")
        ...
        || (lowered.contains("what's up")
            || lowered.contains("whats up")
            || lowered.contains("what is up"))
}
...
fn generate_north_star_opening_text(
    ...
    let (saved_prompt, default_prompt) = if is_outbound_outreach_call(session_context) {
        ...
        (
            &settings.call_inbound_opener_prompt,
            DEFAULT_CALL_INBOUND_OPENER_PROMPT,
        )
    };
...
fn count_lane_matches(text: &str, needles: &[&str]) -> usize {
    needles
        .iter()
...
fn classify_call_answer_lane(transcript_text: &str) -> &'static str {
    let lowered = transcript_text.to_lowercase();

    let relationship_memory_score = count_lane_matches(
        &lowered,
        &[
            "my friends",
            "my friend",
    ...
    let self_identity_score = count_lane_matches(
        &lowered,
        &[
            "about me",
            "know me",
    ...
    ) + usize::from(lowered.contains(" me?"))
        + usize::from(lowered.contains(" me "))
        + usize::from(lowered.starts_with("i "));

    let world_grounding_score = count_lane_matches(
        &lowered,
        &[
            "where am i",
            "where are we",
    ...
    let outreach_purpose_score = count_lane_matches(
        &lowered,
        &[
            "why did you call",
            "why are you calling",
    ...
    if outreach_purpose_score > 0
        && outreach_purpose_score >= self_identity_score
        && outreach_purpose_score >= world_grounding_score
    {
        "outreach_purpose"
    } else if relationship_memory_score > 0
        && relationship_memory_score >= self_identity_score
        && relationship_memory_score >= world_grounding_score
    {
        "relationship_memory"
    } else if world_grounding_score > self_identity_score && world_grounding_score > 0 {
        "world_grounding"
    } else if self_identity_score > 0 {
        "self_identity"
    } else {
        "relational_presence"
    }
...
fn lane_guidance_for_call(lane: &str) -> &'static str {
    match lane {
    "self_identity" => "answer from manual companion context first, then companion interpretation if it helps. keep the answer about the person, their life, relationships, tastes, goals, and patterns. do not drift into location or nearby facts unless the user clearly asked for them or they are directly relevant.",
    "relationship_memory" => "answer from explicit stored relationship facts first. use manual companion context and meaningful place notes only when they clearly name the person or relationship being asked about. do not invent hobbies, anecdotes, personality traits, recent actions, or extra backstory. do not blend multiple friends or family members together. if a detail is not explicitly stored, say you do not have that detail right now.",
    "outreach_purpose" => "answer from outreach purpose first. then use companion interpretation and manual companion context to explain why reaching out made sense for this person and this moment. do not let world facts dominate unless the reason for calling was explicitly about place or conditions.",
    "world_grounding" => "answer from grounded world facts first. only use manual companion context or companion interpretation if it directly helps the world question. do not invent any world fact that is not explicitly present.",
    _ => "treat this as ordinary live conversation. prefer person-centered and relational context over world facts unless the user is clearly asking about location, weather, nearby places, or another grounded outside-world detail.",
  }
}

fn build_lane_aware_call_context(session_context: &str, transcript_text: &str) -> String {
    let lane = classify_call_answer_lane(transcript_text);
    let lane_label = match lane {
        "self_identity" => "self / identity",
        "relationship_memory" => "known people / relationships",
        "outreach_purpose" => "outreach purpose",
        "world_grounding" => "world grounding",
        _ => "relational conversation",
    };
    format!(
        "active answer lane: {lane_label}\nlane guidance: {}\n\n{}",
        lane_guidance_for_call(lane),
        session_context
    )
}
...
fn call_grounding_system_prompt(base_prompt: &str) -> String {
    format!(
    "{base_prompt}\n\nGrounding contract:\n- Treat the call context as the only trusted source of outside-world facts.\n- Treat the manual companion context and meaningful-places section as the only trusted source of stable personal and relationship facts.\n- Only state a fact as true when it is explicitly present in the call context.\n- Do not turn partial clues into factual claims.\n- Do not invent hobbies, stories, personality traits, or relationship dynamics that are not explicitly present in the call context.\n- Do not merge details from different people, different places, or different memories into one blended answer.\n- If something is not explicitly grounded in the call context, say you do not have that information right now.\n- Keep inferences clearly tentative, and never present them as confirmed facts."
  )
}
...
fn fallback_call_reply(transcript_text: &str) -> String {
    let lowered = transcript_text.to_lowercase();
    if ["frog", "frogs", "joke", "funny"]
        .iter()
        .any(|needle| lowered.contains(needle))
    {
        DEFAULT_CALL_REPLY_PLAYFUL.to_string()
    } else if ["was really nice", "good morning", "that was lovely"]
        .iter()
        .any(|needle| lowered.contains(needle))
    {
...
fn resolve_voice_paths(app_data_dir: &Path, settings: &AppSettings) -> VoicePaths {
    ...
    VoicePaths {
        model_path: if settings.tts_model_path.trim().is_empty() {
            voice_root.join(KOKORO_MODEL_FILE)
        } else {
            PathBuf::from(settings.tts_model_path.trim())
        },
        voices_path: if settings.tts_voices_path.trim().is_empty() {
            voice_root.join(KOKORO_VOICES_FILE)
        } else {
            PathBuf::from(settings.tts_voices_path.trim())
        },
...
fn download_file_force(client: &Client, url: &str, destination: &Path) -> Result<(), AppError> {
