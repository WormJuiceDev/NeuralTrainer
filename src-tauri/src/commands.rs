...
use crate::{
    ...
    models::{
        ...
        CallTurnResult, CompanionContextCategory, CompanionContextEntry, CompanionContextSnapshot,
        CompanionHomeSnapshot, CreateCompanionContextCategoryInput,
        ...
        DeleteCompanionContextCategoryInput, DiagnosticStatus, DraftedOutreachAutoDispatchResult,
        DraftedOutreachDispatchResult, EndCallSessionInput, LivedMomentSnapshot,
        LocationEventInput, ManualReflection, MemoryGrowthSnapshot, MemorySystemSnapshot,
        MvpRealityCheckSnapshot, NearbyInterestFilter, NorthStarLiveReplyStreamEvent,
        NorthStarPairingCode, NorthStarRtcIceServer, NorthStarRuntimeSnapshot, NorthStarSnapshot,
        NorthStarTurnProcessingResult, NorthStarWebRtcSignal, PassiveContextSnapshot,
        PhaseOneSnapshot, PhaseThreeSnapshot, Place, ProtectedRule, PushSpeechStreamAudioInput,
        RawLocationEvent, RealWorldPresenceSnapshot, ReorderCompanionContextEntriesInput,
        RunCallTurnInput, SettingsEntry, SimulationRunInput, SimulationRunResult,
        SimulationScenario, SimulationSuiteResult, SpeechStreamSnapshot, StartCallSessionInput,
        StartSpeechStreamInput, StopSpeechStreamInput, UpdateCompanionContextCategoryIconInput,
        UpdateCompanionContextEntryInput, UpdateMemoryItemInput, UpdateNearbyInterestFilterInput,
        UpdatePlaceInput, UpdateRuleInput, VoiceSnapshot, VoiceSynthesisResult,
    },
    north_star,
    state::AppState,
    voice,
...
fn infer_north_star_call_origin_and_purpose(raw_note: &str) -> (&'static str, String) {
    ...
    if trimmed.eq_ignore_ascii_case(NORTH_STAR_PHONE_OUTBOUND_NOTE) {
        (
            ...
            "The user initiated this call from North Star and is looking to talk with you now."
                .into(),
        )
...
fn build_north_star_session_context(
    state: &AppState,
    session: &CallSession,
) -> Result<String, AppError> {
    let call_origin = session_note_field(&session.notes, "call_origin:").unwrap_or_else(|| {
        infer_north_star_call_origin_and_purpose(&session.notes)
            .0
            .to_string()
    });
    ...
    let remote_call_id =
        session_note_field(&session.notes, "remote_call_id:").unwrap_or_else(|| "unknown".into());
    let manual_companion_context = build_north_star_manual_companion_context(state)?;
    let meaningful_places_context = build_north_star_meaningful_places_context(state)?;
    let location_context = build_north_star_location_context(state)?;
    let companion_context = build_north_star_companion_context(state)?;
    Ok(format!(
    "call mode: {}\noutreach purpose: {}\nremote call id: {}\nraw call note: {}\nhandoff kind: {}\noutcome so far: {}\ntranscript summary so far: {}\nmanual companion context:\n{}\nmeaningful places memory:\n{}\ncompanion interpretation:\n{}\nlocation context:\n{}\nrecent exchange:\n{}",
    call_origin,
    outreach_purpose,
    remote_call_id,
    raw_note,
    ...
    manual_companion_context,
    meaningful_places_context,
    companion_context,
    location_context,
...
fn build_north_star_manual_companion_context(state: &AppState) -> Result<String, AppError> {
    let home = db::companion_home_snapshot(&state.db_path)?;
    let mut lines = Vec::new();
    lines.push("stable manually taught context about this person:".into());
    ...
    {
        lines.push(format!("- {}:", section.category.label));
        for entry in section.entries.iter().take(2) {
            entry_count += 1;
            let title = entry.title.trim();
            let body = entry.body.trim().replace('\n', " ");
            if title.is_empty() {
    ...
    if entry_count == 0 {
        lines.push("- no manual companion context is currently stored.".into());
    }

    lines.push("boundary: this section is the best source for stable facts about the person, their relationships, tastes, goals, and home life.".into());
    lines.push("boundary: do not invent hobbies, stories, personality traits, or relationship dynamics that are not explicitly written in this section.".into());
    lines.push("boundary: do not merge two different people into one blended description.".into());
    Ok(lines.join("\n"))
}

fn build_north_star_meaningful_places_context(state: &AppState) -> Result<String, AppError> {
    let snapshot = db::phase_one_snapshot(&state.db_path)?;
    let mut lines = Vec::new();
    lines.push("stored named places and place-linked notes:".into());

    let mut count = 0usize;
    for place in snapshot
        .places
        .iter()
        .filter(|place| place.is_user_named)
        .take(12)
    {
        count += 1;
        lines.push(format!(
            ...
            place.place_kind,
            place.meaning_kind,
            if place.is_protected { "yes" } else { "no" },
        ));
        if let (Some(latitude), Some(longitude)) = (place.latitude, place.longitude) {
            ...
            if let Some(address_summary) =
                db::place_address_summary(&state.db_path, latitude, longitude)?
            {
                lines.push(format!("  - grounded address: {}", address_summary));
            }
        }
        if !place.notes.trim().is_empty() {
            lines.push(format!(
                "  - stored note: {}",
                place.notes.trim().replace('\n', " ")
            ));
        }
    }

    if count == 0 {
        lines.push("- no named places are currently stored.".into());
    }

    lines.push(
        "boundary: use this section only for exact stored place facts and exact stored notes."
            .into(),
    );
    lines.push("boundary: do not turn a place note into extra biography, hobbies, stories, recent activity, or personality traits unless those details are explicitly written.".into());
    lines.push("boundary: do not merge details from different places or different people.".into());
    Ok(lines.join("\n"))
}

fn build_north_star_companion_context(state: &AppState) -> Result<String, AppError> {
    let settings = db::load_settings(&state.db_path)?;
    let lived_moment = db::lived_moment_snapshot(&state.db_path, &settings.timezone)?;
    let mut lines = Vec::new();

    lines.push("current lived interpretation for this person and moment:".into());
    lines.push(format!("- lived moment summary: {}", lived_moment.summary));
    lines.push(format!(
        "- recent contact summary: {}",
        lived_moment.recent_contact_summary
    ));
    lines.push(format!(
    "- current read: primary assessment {} / recommended signal {} / recommended contact mode {} / action bias {}",
    lived_moment.primary_assessment,
    lived_moment.recommended_signal,
    lived_moment.recommended_contact_mode,
    lived_moment.action_bias,
  ));
    lines.push(format!(
        "- rhythm state: {} / hint {} / likely sleep window {}",
        lived_moment.rhythm_state,
        lived_moment.contact_rhythm_hint,
        if lived_moment.is_likely_sleep_window {
            "yes"
        } else {
            "no"
        },
    ...
    if let Some(place) = lived_moment.matched_place.as_ref() {
        lines.push(format!(
            "- matched meaningful place for this person: {} / {} / {}",
            place.label, place.place_kind, place.meaning_kind,
        ));
    } else if let Some(place) = lived_moment.repeated_place.as_ref() {
        lines.push(format!(
            "- repeated place pattern for this person: {} / {} visits / average stay {} seconds",
            place.label, place.visit_count, place.average_duration_seconds,
        ));
    }

    if let Some(opportunity) = lived_moment.opportunities.first() {
        lines.push(format!(
            "- strongest opportunity signal: {}",
            opportunity.summary
        ));
    }
    if let Some(safeguard) = lived_moment.safeguards.first() {
        lines.push(format!(
            "- strongest safeguard signal: {}",
            safeguard.summary
        ));
    }
    if let Some(bridge) = lived_moment.relational_bridges.first() {
        lines.push(format!(
            "- strongest relational bridge: {} / {} / reason {}",
            bridge.title, bridge.bridge_kind, bridge.reason,
        ));
    }

    lines.push("boundary: this section is companion interpretation about the person and moment, not raw outside-world fact.".into());
    Ok(lines.join("\n"))
}

fn build_north_star_location_context(state: &AppState) -> Result<String, AppError> {
    let world = db::get_real_world_presence_snapshot(&state.db_path)?;
    let mut lines = Vec::new();

    lines.push("grounded world facts available right now:".into());
    lines.push(format!("- world read captured at: {}", world.captured_at));
    lines.push(format!("- {}", world.location_summary));

    if let (Some(latitude), Some(longitude)) = (world.latitude, world.longitude) {
        lines.push(format!(
            "- grounded coordinates from the latest stored location pulse: {:.5}, {:.5}",
            latitude, longitude,
        ));
    } else {
        lines.push(
            "- grounded coordinates from the latest stored location pulse: unavailable".into(),
        );
    }

    if let Some(weather) = world.weather.as_ref() {
        lines.push(format!("- weather: {}", weather.summary));
        lines.push(format!(
      "- weather details: temperature {}C / feels like {}C / wind {} kph / precipitation chance {}%",
      weather.temperature_celsius.map(|value| format!("{value:.1}")).unwrap_or_else(|| "unknown".into()),
      weather.apparent_temperature_celsius.map(|value| format!("{value:.1}")).unwrap_or_else(|| "unknown".into()),
      weather.wind_speed_kph.map(|value| format!("{value:.1}")).unwrap_or_else(|| "unknown".into()),
      weather.precipitation_probability_percent.map(|value| format!("{value:.0}")).unwrap_or_else(|| "unknown".into()),
    ));
    } else {
        lines.push("- weather: unavailable".into());
    }

    if let Some(daylight) = world.daylight.as_ref() {
        lines.push(format!("- daylight: {}", daylight.summary));
    } else {
        lines.push("- daylight: unavailable".into());
    }

    lines.push(format!("- warnings: {}", world.warning_summary));

    if world.nearby_candidates.is_empty() {
        lines.push("- nearby grounded candidates: none".into());
    } else {
        lines.push("- nearby grounded candidates:".into());
        for candidate in world.nearby_candidates.iter().take(8) {
            let distance = candidate
                .distance_meters
                .map(|value| format!("{} meters", value.round() as i64))
                .unwrap_or_else(|| "unknown distance".into());
            lines.push(format!(
                "  - {} / {} / {} / {}",
                candidate.title, candidate.category_key, distance, candidate.summary,
            ));
        }
    }

    lines.push("boundary: use only the explicit world facts listed above.".into());
    lines.push("boundary: do not infer roads, canals, neighborhoods, business opening hours, or specific nearby services unless they are explicitly listed above.".into());
...
fn refresh_north_star_call_location_context_impl(
    ...
    if settings.north_star_user_handle.trim().is_empty()
        || settings.north_star_device_token.trim().is_empty()
    {
        return Ok(());
    }

    match north_star::request_location_pulse(&settings) {
        Ok(_) => state.push_event(format!(
            "Asked North Star for a fresh location pulse for {reason}."
        )),
        Err(caught) => {
            state.push_event(format!(
                "North Star call location pulse could not be requested for {reason}: {caught}"
            ));
            return Ok(());
        }
    }
    ...
    for attempt in 0..3 {
        ...
        match north_star::pull_location_events(&state.db_path, &mut settings) {
            Ok(snapshot) => {
                ...
                state.push_event(format!(
                    "North Star call location refresh ({reason}): {}",
                    snapshot.detail
                ));
                let _ = db::get_real_world_presence_snapshot(&state.db_path);
                if !snapshot
                    .detail
                    .starts_with("No new North Star location events")
                    || attempt == 2
                {
                    break;
                }
...
fn schedule_north_star_call_location_refresh(state: &AppState, reason: &str) {
    ...
    async_runtime::spawn(async move {
        ...
        let _ = async_runtime::spawn_blocking(move || {
            refresh_north_star_call_location_context_impl(&refresh_state, &refresh_reason)
        })
        .await;
    });
}
...
pub fn export_memory_intel(
    ...
    let result = db::export_memory_intel(&state.db_path, &destination_path)?;
    state.push_event(format!("Exported memory intel to {}.", result.path));
    Ok(result)
}
...
pub fn import_memory_intel(
    ...
    let result = db::import_memory_intel(&state.db_path, &source_path)?;
    state.push_event(format!("Imported memory intel from {}.", result.path));
    Ok(result)
}
...
pub fn export_system_settings(
    ...
    let result = db::export_system_settings(&state.db_path, &destination_path)?;
    state.push_event(format!("Exported system settings to {}.", result.path));
    Ok(result)
}
...
pub fn import_system_settings(
    ...
    let result = db::import_system_settings(&state.db_path, &source_path)?;
    state.push_event(format!("Imported system settings from {}.", result.path));
    Ok(result)
}
...
pub fn create_companion_context_category(
    ...
    state.push_event(format!(
        "Added companion context category '{}'.",
        category.label
    ));
    Ok(category)
}
...
pub fn update_companion_context_category_icon(
    ...
    state.push_event(format!(
        "Updated companion context category icon for '{}'.",
        category.label
    ));
    Ok(category)
}
...
pub fn update_companion_context_entry(
    ...
    state.push_event(format!(
        "Updated companion context entry '{}'.",
        entry.title
    ));
    Ok(entry)
}
...
pub fn archive_companion_context_entry(
    ...
    state.push_event(format!(
        "Archived companion context entry '{}'.",
        entry.title
    ));
    Ok(entry)
}
...
pub fn delete_companion_context_category(
    ...
    state.push_event(format!(
        "Deleted companion context category '{}'.",
        payload.category_key
    ));
    Ok(snapshot)
}
...
#[tauri::command]
pub fn get_north_star_runtime_snapshot(
    state: State<'_, AppState>,
) -> Result<NorthStarRuntimeSnapshot, AppError> {
    let settings = db::load_settings(&state.db_path)?;
...
#[tauri::command]
pub fn bind_north_star_desktop(state: State<'_, AppState>) -> Result<NorthStarSnapshot, AppError> {
    let mut settings = db::load_settings(&state.db_path)?;
...
pub fn auto_dispatch_eligible_outreach(
    ...
    if let Some(outreach) = candidate {
        ...
        return Ok(DraftedOutreachAutoDispatchResult {
            dispatched: true,
            detail: format!(
                "Sent the next low-friction draft automatically. {}",
                dispatch.north_star_detail
            ),
            dispatch: Some(dispatch),
            held_outreach_event: None,
            eligibility_reason: None,
...
fn process_next_north_star_call_turn_impl(
    ...
    let active_session = session_snapshot.active_session.clone().filter(|session| {
        session.handoff_kind == "north_star_companion" && session.session_state == "active"
    });

    let result = north_star::process_next_pending_turn(&settings, |_call, turn| {
      ...
      AppError::Message("Start the accepted North Star call on the desktop before processing remote speech turns.".into())
        ...
        let session_context = build_north_star_session_context(state, &local_session)?;
        let input_audio = base64::engine::general_purpose::STANDARD
            .decode(turn.input_audio_base64.as_bytes())
            .map_err(|caught| {
                AppError::Message(format!("North Star turn audio was invalid: {caught}"))
            })?;
...
pub async fn process_next_north_star_call_turn(
    ...
    async_runtime::spawn_blocking(move || process_next_north_star_call_turn_impl(&state))
        .await
        .map_err(|caught| {
            AppError::Message(format!("North Star turn processor task failed: {caught}"))
        })?
}
...
fn process_north_star_live_turn_impl(
    ...
    let session = session_snapshot.active_session.ok_or_else(|| {
        AppError::Message("There is no active North Star live call session.".into())
    })?;

    if session.handoff_kind != "north_star_companion" {
        return Err(AppError::Message(
            "The active session is not a North Star call.".into(),
        ));
    }
    ...
    let input_audio = base64::engine::general_purpose::STANDARD
        ...
        .map_err(|caught| {
            AppError::Message(format!("North Star live audio was invalid: {caught}"))
        })?;
    ...
    state.push_event(format!(
        "Processed a live North Star turn for session {}.",
        session.id
    ));
    Ok(result)
}
...
pub async fn process_north_star_live_turn(
    ...
    async_runtime::spawn_blocking(move || {
        process_north_star_live_turn_impl(&state, session_id, audio_base64)
    })
    .await
...
fn stream_north_star_live_turn_impl(
            ...
            app.emit("north-star-live-reply-stream", &event)
                .map_err(|caught| {
                    AppError::Message(format!(
                        "Could not emit North Star live reply event: {caught}"
                    ))
                })?;
            Ok(())
        },
    ...
    state.push_event(format!(
        "Streamed a live North Star turn for session {}.",
        session.id
    ));
    Ok(())
}
...
fn complete_north_star_live_speech_stream_impl(
    ...
    let result = voice::stop_speech_stream_and_stream_reply(
        ...
        &request_id,
        |event: NorthStarLiveReplyStreamEvent| {
            handler.send(event).map_err(|caught| {
                AppError::Message(format!(
                    "Could not send North Star live reply event over channel: {caught}"
                ))
            })?;
            Ok(())
        },
    ...
    state.push_event(format!(
        "Completed a streamed North Star speech turn for session {}.",
        session.id
    ));
    Ok(())
}
...
pub async fn start_north_star_live_turn_stream(
    ...
    let state = state.inner().clone();
    async_runtime::spawn_blocking(move || {
        let result = stream_north_star_live_turn_impl(
            app.clone(),
            &state,
            session_id,
            audio_base64,
...
pub async fn complete_north_star_live_speech_stream(
    ...
    let state = state.inner().clone();
    async_runtime::spawn_blocking(move || {
        let result = complete_north_star_live_speech_stream_impl(
            &state,
            session_id,
            request_id.clone(),
            &handler,
...
#[tauri::command]
pub fn download_kokoro_assets(state: State<'_, AppState>) -> Result<VoiceSnapshot, AppError> {
    let settings = db::load_settings(&state.db_path)?;
    ...
    let worker_loaded = state
        ...
        .map_err(|_| AppError::Message("Voice worker mutex was poisoned.".into()))?
        .is_some();
    let snapshot = voice::voice_snapshot(
        state.app_data_dir.as_ref().as_path(),
        &settings,
        worker_loaded,
        &state.kokoro_fastapi_runtime,
...
#[tauri::command]
pub fn prepare_kokoro_runtime(state: State<'_, AppState>) -> Result<VoiceSnapshot, AppError> {
    let settings = db::load_settings(&state.db_path)?;
...
#[tauri::command]
pub fn setup_local_speech(state: State<'_, AppState>) -> Result<VoiceSnapshot, AppError> {
    let settings = db::load_settings(&state.db_path)?;
    voice::setup_local_speech(
        state.app_data_dir.as_ref().as_path(),
        &settings,
        &state.speech_stream_worker,
    )?;
...
pub fn synthesize_voice_preview(
    ...
    state.push_event(format!(
        "Synthesized Kokoro voice preview at {}.",
        result.output_path
    ));
    Ok(result)
}
...
pub fn synthesize_north_star_phrase(
    ...
    state.push_event(format!(
        "Synthesized North Star line at {}.",
        result.output_path
    ));
    Ok(result)
}
...
pub fn synthesize_north_star_opening(
    ...
    state.push_event(format!(
        "Synthesized North Star opening at {}.",
        result.output_path
    ));
    Ok(result)
}
...
pub fn start_speech_stream(
    ...
    let settings = db::load_settings(&state.db_path)?;
    let call_session_snapshot =
        db::call_session_snapshot(&state.db_path, Some(payload.session_id))?;
    if call_session_snapshot.active_session.is_none() {
        return Err(AppError::Message(
            "There is no active call session to listen on.".into(),
        ));
    }
    ...
    state.push_event(format!(
        "Started live speech stream for session {}.",
        payload.session_id
    ));
    Ok(SpeechStreamSnapshot {
        active: snapshot.active,
...
pub fn stop_speech_stream_and_reply(
    ...
    state.push_event(format!(
        "Stopped live speech stream for session {} and generated a reply.",
        payload.session_id
    ));
    Ok(result)
}
...
pub fn push_speech_stream_audio(
    ...
    if call_session_snapshot.active_session.is_none() {
        return Err(AppError::Message(
            "There is no active call session to feed.".into(),
        ));
    }
...
pub fn run_call_turn(
    ...
    let session_snapshot = db::call_session_snapshot(&state.db_path, Some(payload.session_id))?;
    let session = session_snapshot.active_session.ok_or_else(|| {
        AppError::Message("There is no active call session to use for a spoken turn.".into())
    })?;
    let session_context = format!(
    "handoff kind: {}\nnotes: {}\noutcome so far: {}\ntranscript summary so far: {}\nrecent exchange:\n{}",
    session.handoff_kind,
...
#[tauri::command]
pub fn get_phase_one_snapshot(state: State<'_, AppState>) -> Result<PhaseOneSnapshot, AppError> {
...
pub fn update_nearby_interest_filter(
    ...
    state.push_event(format!(
        "Updated nearby-interest filter '{}'.",
        payload.label
    ));
    Ok(filters)
}
...
#[tauri::command]
pub fn run_message_decisions(state: State<'_, AppState>) -> Result<DecisionRunResult, AppError> {
...
#[tauri::command]
pub fn get_decision_snapshot(state: State<'_, AppState>) -> Result<DecisionSnapshot, AppError> {
